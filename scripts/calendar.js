class Application {
  ...
  renderMonthSearch() {
    Handlebars.registerPartial(
      ...
      $('#month_search_template').html()
    );
    let monthSearchTemplate = Handlebars.compile(
      $('#months_search_template').html()
    );
  ...
  renderDateSearch(month = this.calendar) {
    Handlebars.registerPartial(
      ...
      $('#date_search_template').html()
    );
    let dateSearchTemplate = Handlebars.compile(
      $('#dates_search_template').html()
    );
  ...
  renderMonthDropdown() {
    Handlebars.registerPartial(
      ...
      $('#month_dropdown_template').html()
    );
    let monthDropdownTemplate = Handlebars.compile(
      $('#months_dropdown_template').html()
    );

    $('#month_dropdown').html(
      monthDropdownTemplate({ calendar: this.calendar })
    );
  }
  ...
  prevMonth() {
    ...
    $('.prev').on('click', function(e) {
      let currentMoIdx = self.monthList.indexOf($('#month_dropdown')[0].value);
      if (currentMoIdx === 0) {
        currentMoIdx = 12;
      }
      $('#month_dropdown').val(self.monthList[currentMoIdx - 1]);
      self.renderCalendar(self.chooseMonth(self.monthList[currentMoIdx - 1]));
    });
  }
  ...
  findDate(event) {
    ...
    let datesObj = this.calendar.filter(function(obj) {
      if (obj['name'] === $($date[0]).attr('data-month')) {
        return obj;
      }
    })[0]['dates'];
    return datesObj.filter(function(obj) {
      if (obj['date'] === +$($date[0]).attr('data-date')) {
        return obj;
      }
    })[0];
  }
  ...
  modalEvents(dayObj) {
    ...
    $('.add').on('click', function(e) {
      ...
      $('#modal ul').append(
        `<li data-id=${dayObj['id']}><p>${
          $('.todo_input')[0].value
        }</p><div class='material-icons delete'>delete</div></li>`
      );
      dayObj['id']++;
      $('.todo_input')[0].value = '';
